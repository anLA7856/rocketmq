...
public class ConsumeMessageConcurrentlyService implements ConsumeMessageService {
    ...
    public void submitConsumeRequest(
        ...
        if (msgs.size() <= consumeBatchSize) {
            ...
            /**
             * 如果拉取的消息条数大于consumeMessageBatchMaxSize ， 则对拉取消息进行分
             * 页，每页consumeMessag巳BatchMaxSize 条消息，创建多个ConsumeReq uest 任务并提交到
             * 消费线程池。Consum巳Request 的run 方法封装了具体消息消费逻辑。
             */
    ...
    public void processConsumeResult(
        ...
        /**
         * 根据消息监昕器返回的结果， 计算ackl nd ex ，如果返回C ONSUME _SUCCES S,
         * ack lndex 设置为msgs.s i ze （） 斗，如果返回RECONSUME_LATER, acklndex = -1 ， 这是为下
         * 文发送msg back ( ACK ）消息做准备的。
         */
        switch (status) {
            case CONSUME_SUCCESS:
        ...
        /**
         * 如果是集群模式， 业务方返回RECONSUME_LATER ，消息并不会重新被消
         * 费，只是以警告级别输出到日志文件。如果是集群模式， 消息消费成功，由于acklndex=
         * consumeRequest.getMsgs().size （）-1， 故i=acklndex+ 1 等于consumeRequest.getMsgs().size() ,
         * 并不会执行sendMessageBack 。只有在业务方返回RECONSUME_LATER 时，该批消息都
         * 需要发ACK 消息，如果消息发送ACK 失败，则直接将本批ACK 消费发送失败的消息再
         * 次封装为ConsumeRequest ，然后延迟Ss 后重新消费。如果ACK 消息发送成功，则该消息
         * 会延迟消费。
         */
        ...
        /**
         * 从Process Queue 中移除这批消息， 这里返回的偏移量是移除该批消息后最
         * 小的偏移量，然后用该偏移量更新消息消费进度，以便在消费者重启后能从上一次的消费
         * 进度开始消费，避免消息重复消费。值得重点注意的是当消息监听器返回RECONSUME
         * LATER ，消息消费进度也会向前推进，用Proces sQueue 中最小的队列偏移量调用消息消费
         * 进度存储器OffsetStore 更新消费进度，这是因为当返回RECONSUME_LATER, RocketMQ
         * 会创建一条与原先消息属性相同的消息，拥有一个唯一的新msgld ，并存储原消息ID ，该
         * 消息会存入到commitlog 文件中，与原先的消息没有任何关联，那该消息当然也会进入到
    ...
    class ConsumeRequest implements Runnable {
        ...
        public void run() {
            /**
             * ：进入具体消息消费时会先检查processQueue 的dropped ，如果设置为true ， 则
             * 停止该队列的消费，在进行消息重新负载时如果该消息队列被分配给消费组内其他消费者
            ...
            try {
                ...
                // ：执行具体的消息消费，调用应用程序消息监昕器的consumeMessage 方法，
                //进入到具体的消息消费业务逻辑，返回该批消息的消费结果。最终将返回C ONSUME
                //SUCCESS （消费成功）或RECONSUME LATER （需要重新消费） 。
            ...
            /**
             * 执行业务消息消费后，在处理结果前再次验证一下ProcessQueue 的isDroped
             * 状态值，如果设置为tru巳，将不对结果进行处理，也就是说如果在消息消费过程中进入到
