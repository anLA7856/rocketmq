...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * 根据消息进度从消息服务器拉取不到消
     * 息时重新计算消费策略。
     * CONSUME FROM LAST OF FSE T ：从队列当前最大偏移量开始消费。
     * CONSUME FROM FIRST OFFSET ：从队列当前最小偏移量开始消费。
     * CONSUME FROM TIMESTAMP ：从消费者启动时间戳开始消费。
     * 注意：如果从消息进度服务OffsetStore 读取到MessageQu eue 中的偏移量不小于0 ，则
     ...
     * 故消费者线程个数其实最多只有co nsumeThreadMin 个。
     *
     */
     ...
     *
     * 并发消息消费时处理队列最大跨度，默认2 000 ,
     * 表示如果消息处理队列中偏移量最大的消息与偏移量最小的消息的跨度超过2 000 则延迟到
     * 毫秒后再拉取消息。
     *
     *
     */
     ...
     * 就是每次传入MessageListtener# consumeMessage 中的消息条数。
     *
     *
     */
     ...
     *
     *
     * 最大消费重试次数。如果消息消费次数超过maxR巳consume Times
     * 还未成功，则将该消息转移到一个失败队列，等待被删除。
     *
     */
